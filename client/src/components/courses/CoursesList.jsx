  ...
  if (courses.length === 0) {
    return (
      <div className="py-16 text-center rounded-3xl bg-white dark:bg-[#111418] border border-gray-200 dark:border-white/[0.08] shadow-sm text-gray-500 dark:text-[#8C877D] space-y-3">
        ...
        <p className="text-sm font-semibold text-gray-900 dark:text-[#F5F1E8]">No courses match your selected filters</p>
        <p className="text-xs text-gray-500 dark:text-[#8C877D]">Try adjusting your search query or reset category filters.</p>
      </div>
    );
  }
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {courses.map((course) => {
        ...
        return (
          <div
            key={course.id}
            className="m3-card p-5 sm:p-6 flex flex-col justify-between group bg-white dark:bg-[#111418] border border-gray-200 dark:border-white/[0.08] hover:border-[#FF6B5F]/40"
          >
            <div>
              {/* Header Badge & Rating */}
              ...
              <h3
                ...
                className="text-base sm:text-lg font-bold text-gray-900 dark:text-[#F5F1E8] mb-2 line-clamp-2 hover:text-[#FF857A] transition-colors cursor-pointer"
              >
                {course.title}
              </h3>
              ...
              {/* Meta Info */}
              <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500 dark:text-[#8C877D] mb-4 pb-3 border-b border-gray-200 dark:border-white/[0.06]">
                <span className="flex items-center gap-1 text-[#FF857A] font-semibold">
                  <BookOpen className="w-3.5 h-3.5 text-[#FF6B5F]" />
                  {course.platform}
                </span>
                <span className="flex items-center gap-1">
                  <Clock className="w-3.5 h-3.5 text-gray-500 dark:text-[#8C877D]" />
                  {course.duration}
                </span>
                <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-gray-100 dark:bg-white/5 text-gray-600 dark:text-[#C7C2B6] border border-gray-200 dark:border-white/10">
                  {course.difficulty}
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-1.5 mb-5">
                {course.skillsCovered?.slice(0, 3).map((s, i) => (
                  <span
                    key={i}
                    className="px-2.5 py-0.5 rounded-md text-[11px] font-medium bg-gray-50 dark:bg-[#16191E] text-gray-600 dark:text-[#C7C2B6] border border-gray-200 dark:border-white/[0.08]"
                  >
                    {s}
                  </span>
                ))}
              ...
              {course.enrolled ? (
                <div className="space-y-2">
                  <div className="flex justify-between items-center text-xs">
                    <span className="text-gray-500 dark:text-[#8C877D] font-medium">
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    ...
                    {course.assessmentId && (
                      <button
                        ...
                        className="p-2.5 rounded-xl border border-gray-200 dark:border-white/[0.08] hover:border-[#FF6B5F]/40 bg-gray-50 dark:bg-[#16191E] text-[#FF857A] transition-colors cursor-pointer"
                      >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => onOpenDetails(course)}
                    className="p-2.5 rounded-xl border border-gray-200 dark:border-white/[0.08] hover:border-gray-200 dark:hover:border-white/20 bg-gray-50 dark:bg-[#16191E] text-gray-600 dark:text-[#C7C2B6] hover:text-gray-900 dark:hover:text-[#F5F1E8] transition-colors cursor-pointer"
                    title="View Syllabus"
                  >
