...
export default function AssessmentsList({
  ...
  if (assessments.length === 0) {
    return (
      <div className="py-16 text-center rounded-3xl bg-[#111418] border border-white/[0.08] shadow-sm text-[#8C877D] space-y-3">
        ...
        <p className="text-sm font-semibold text-[#F5F1E8]">No assessments found matching your filter</p>
        <p className="text-xs text-[#8C877D]">Try choosing a different category or generate a custom AI assessment.</p>
      </div>
    );
  }
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      default:
        return (
          <span className="px-2.5 py-1 rounded-full text-[11px] font-bold bg-white/5 text-[#8C877D] border border-white/10 flex items-center gap-1 font-mono">
            <Lock className="w-3 h-3 text-[#8C877D]" />
            Locked
          </span>
        );
    }
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {assessments.map((assessment) => {
        ...
        return (
          <div
            key={assessment.id}
            className={`m3-card p-5 sm:p-6 flex flex-col justify-between group bg-[#111418] border transition-all ${
              isLocked
                ? 'opacity-60 border-white/[0.04]'
                : isPassed
                ? 'border-[#34D399]/30 hover:border-[#34D399]/60'
                : 'border-white/[0.08] hover:border-[#FF6B5F]/40'
            }`}
          >
            <div>
              ...
              <div className="flex items-center justify-between gap-2 mb-3">
                ...
                <span className="px-2.5 py-0.5 rounded-lg text-xs font-bold bg-white/5 border border-white/10 text-[#C7C2B6] font-mono">
                  {assessment.difficulty}
                </span>
              </div>

              {/* Assessment Title */}
              <h3 className="text-base sm:text-lg font-bold text-[#F5F1E8] mb-2 line-clamp-2">
                {assessment.title}
              </h3>
              ...
              {/* Meta Info */}
              <div className="space-y-1.5 text-xs text-[#8C877D] mb-4 pb-3 border-b border-white/[0.06]">
                <div className="flex items-center justify-between">
                  ...
                  <span className="flex items-center gap-1.5">
                    <HelpCircle className="w-3.5 h-3.5 text-[#8C877D]" />
                ...
                <div className="flex items-center justify-between pt-1">
                  <span className="text-[#8C877D]">
                    Passing Benchmark: <strong className="text-[#F5F1E8] font-mono">{assessment.passingScore}%</strong>
                  </span>
              ...
              {isPassed ? (
                ...
                <div className="p-2.5 rounded-xl bg-white/[0.03] border border-white/[0.06] text-[#8C877D] text-xs flex items-center gap-2 mb-4">
            ...
            <div className="pt-2">
              {isLocked ? (
                <button
                  disabled
                  className="w-full py-2.5 px-4 rounded-xl bg-white/5 text-[#8C877D] text-xs font-bold cursor-not-allowed flex items-center justify-center gap-1.5"
                >
                ...
                <button
                  ...
                  className="w-full py-2.5 px-4 rounded-xl border border-white/[0.1] bg-[#16191E] hover:bg-white/5 text-[#F5F1E8] text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                >
